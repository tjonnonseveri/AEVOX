...
const V = (window.V = {
	...
	async renderChat(el, data) {
      ...
      <div class="chat-window">
		...
		onValue(msgsRef, (snap) => {
			...
			if (!data2) {
				container.innerHTML =
					'<div class="empty-state" style="padding:24px"><p>Aucun message. Dites bonjour !</p></div>';
				return;
			}
		...
		onValue(typingRef, (snap) => {
			...
			if (ti)
				ti.textContent =
					snap.val() === true
						? `${other.name} est en train d'\u00e9crire...`
						: "";
		});
            ...
            <div style="display:flex;gap:8px;margin-top:8px">
              ...
              <button class="btn-secondary" style="font-size:12px;padding:6px 14px"
                ...
                &#10005; Refuser
							...
							text = `<span>${esc(n.fromName || "?")}</span> a aim\u00e9 votre publication`;
						else if (n.type === "follow")
							text = `<span>${esc(n.fromName || "?")}</span> vous suit maintenant`;
						else if (n.type === "comment")
							text = `<span>${esc(n.fromName || "?")}</span> a comment\u00e9 votre publication`;
	...
	async adminGroupMembers(groupId, groupName, members) {
		...
		const nonMembers = allUsers.filter(
			(u) => !members.includes(u.id) && !u.banned && u.id !== currentUser.id,
		);
	...
	async acceptGroupInvite(notifId, groupId) {
		...
		setTimeout(
			() => this.go("chat", { uid: "__group__", cid: groupId, groupId }),
			300,
		);
	},
	...
	async cleanupUserRelations(uid) {
		try {
			...
			const groupBatch = groupsSnap.docs
				...
				.map((d) =>
					updateDoc(doc(db, "groups", d.id), { members: arrayRemove(uid) }),
				);
